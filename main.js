...
// ---------------------------------------------------------------------
// CONFIG — API_BASE_URL is normally set automatically: the bot embeds
// its own address into every payment link it generates (as ?api=...),
// so this reads it from the URL. The line below is only a fallback for
// local testing/previewing this page without a real link from Discord.
// ---------------------------------------------------------------------
const FALLBACK_API_BASE_URL = "https://your-bot-hosting-url.example.com";
...
const API_BASE_URL = params.get("api") || sessionStorage.getItem("apiBaseUrl") || FALLBACK_API_BASE_URL;
if (params.get("api")) sessionStorage.setItem("apiBaseUrl", params.get("api"));
